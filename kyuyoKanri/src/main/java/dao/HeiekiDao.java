...
public class HeiekiDao {
	...
	/**
	 * HeiekiDao의 인스턴스를 반환합니다.
	 * HeiekiDaoのインスタンスを返します。
	 *
	...
	public int insertHeieki(Connection conn, Heieki heieki) throws SQLException {
		...
		try (PreparedStatement ps = conn.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS)) {
			...
			ps.setDate(4, heieki.getFukumukiKaishi() != null ? new java.sql.Date(heieki.getFukumukiKaishi().getTime()) : null);
			ps.setDate(5, heieki.getFukumukiShuuryou() != null ? new java.sql.Date(heieki.getFukumukiShuuryou().getTime()) : null);
	...
	/**
	 * 특정 ID로 병역 정보를 조회하는 메서드입니다.
	 * 特定のIDで兵役情報を照会するメソッドです。
	 *
	 * @param conn      데이터베이스 연결 객체 / データベース接続オブジェクト
	...
	public Heieki getHeiekiById(Connection conn, int heieki_id) throws SQLException {
		...
		try (PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			if (rs.next()) {
				...
				heieki.setFukumukiKaishi(rs.getDate("fukumukiKaishi") != null ? new Date(rs.getDate("fukumukiKaishi").getTime()) : null);
				heieki.setFukumukiShuuryou(rs.getDate("fukumukiShuuryou") != null ? new Date(rs.getDate("fukumukiShuuryou").getTime()) : null);
	 ...
	 * @throws SQLException SQL 처리 중 오류 발생 시 / SQL処理中にエラーが発生した場合
	 */
	public List<Heieki> selectAllHeieki(Connection conn) throws SQLException {
		String sql = "SELECT h.*, s.namae_kana FROM Heieki h JOIN Shain s ON h.shain_id = s.shain_id";
		List<Heieki> heiekiList = new ArrayList<>();
